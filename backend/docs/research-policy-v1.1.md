# KAIRO Strategy 001 Research Contract v1.1 — Q1 2024

Policy ID: RESEARCH-POLICY-v1.1  
Status: FROZEN  
...
* **Broker Fee Model:**
  ...
  * Per-Side Ticket-Fee Floor:
    $$\text{Fee}_{\text{side}} = \max(\$1.00, \$0.70 \times Q_t)$$
    where $\$0.70 = \$0.65$ base commission $+ \$0.05$ regulatory pass-through.
  * Total Round-Trip Fee:
    $$\text{Fee}_{\text{round trip}} = \text{Fee}_{\text{entry}} + \text{Fee}_{\text{exit}}$$
    For example, $Q_t = 1$ produces $\$1.00$ per side and a $\$2.00$ round trip; $Q_t = 2$ produces $\$1.40$ per side and a $\$2.80$ round trip.
...
* **Exit Rules (Evaluated in strict ordinal priority):**
  Intra-interval high, low, trade tick, reconstructed path, or assumed order-of-events are strictly forbidden. Stop-loss and profit-target conditions are evaluated solely using the closing Bid quote of each completed 1-minute option interval:
  $$\text{STOP}(t) \iff \text{Bid}_{\text{close}}(t) \le 0.80 \times P_{\text{entry}}$$
  $$\text{TARGET}(t) \iff \text{Bid}_{\text{close}}(t) \ge 1.20 \times P_{\text{entry}}$$
  The position exits at that completed interval's closing Bid when either condition is satisfied.
  1. **Catastrophic Stop Loss ($SL$):** Evaluate $\text{STOP}(t)$ first; when satisfied, exit at that completed interval's closing Bid.
  2. **Profit Target ($PT$):** Evaluate $\text{TARGET}(t)$ second; when satisfied, exit at that completed interval's closing Bid.
...
For all signals sharing an identical legal execution timestamp:
$$B_t = \text{account equity immediately before that timestamp's signal batch begins}$$
$$A_t = 0.25 \times B_t$$

Where $A_t$ is the maximum capital allocation for each position. Both $B_t$ and $A_t$ are frozen from account equity immediately before that timestamp's signal batch begins.

Signals are processed sequentially in lexicographically ascending order: `SQQQ` then `TQQQ`. No signal may reserve capital prior to its turn.
...
After each entry, Available Cash is decremented by that trade's all-in cash requirement:
$$\text{Available Cash}_{t,\text{after}} = \text{Available Cash}_{t,\text{before}} - (P_{\text{ask}} \times 100 \times Q_t + \text{Fee}_{\text{entry}})$$

Each subsequent signal in the batch recomputes $Q_t$ using the frozen $A_t$ and updated Available Cash.
...
* **File Location:** `backend/docs/research-policy-v1.1.md`
